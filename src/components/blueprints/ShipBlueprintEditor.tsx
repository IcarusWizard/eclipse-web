...
import { Zap, Shield, Crosshair, Heart, Navigation, Plus, Trash2, CheckCircle2, AlertTriangle, X, Lock } from 'lucide-react';

interface ShipBlueprintEditorProps {
  player: PlayerState;
  onSaveBlueprint: (shipType: ShipType, newSlots: (ShipPart | null)[]) => void;
  onClose: () => void;
}
...
export const ShipBlueprintEditor: React.FC<ShipBlueprintEditorProps> = ({
  ...
  const [draftBlueprints, setDraftBlueprints] = useState<Record<string, (ShipPart | null)[]>>(() => ({
  ...
  const handleSave = () => {
    onSaveBlueprint(activeShipType, currentSlots);
  };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden text-slate-100">
          ...
          {(['interceptor', 'cruiser', 'dreadnought', 'starbase'] as ShipType[]).map((type) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2.5 rounded-t-lg font-bold text-sm tracking-wide transition-all border-t border-x ${
                  isActive
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 overflow-y-auto">
                ...
                {currentSlots.map((part, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative min-h-[96px] rounded-lg border-2 p-2.5 flex flex-col justify-between cursor-pointer transition-all ${
                        isSelected
          ...
          <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 flex flex-col">
            <h3 className="text-xs font-bold text-cyan-400 tracking-wider mb-2 uppercase">
              Component Supply
            </h3>
            ...
            <div className="space-y-2 overflow-y-auto max-h-[380px] pr-1">
              {Object.values(SHIP_PARTS).map((part) => {
                ...
                const isAncient = ANCIENT_PART_IDS.includes(part.id);
                const isAncientUnlocked = isAncient && (player.unlockedAncientParts || []).includes(part.id);
                ...
                const canEquip = isUnlocked && selectedSlotIndex !== null;
                      ...
                      <span className="flex items-center gap-1.5">
                        ...
                        {!isUnlocked && (
                          <span className="text-[9px] text-slate-500 flex items-center gap-0.5">
                            <Lock className="w-2.5 h-2.5 inline" /> {isAncient ? 'Discovery' : 'Research'}
                          </span>
                        )}
                      </span>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 flex justify-between items-center">
          <div className="text-xs text-slate-400">
            {stats.isValid ? (
              <span className="text-emerald-400 flex items-center gap-1">
                <CheckCircle2 className="w-4 h-4" /> Valid Blueprint Configuration
              </span>
            ) : (
              <span className="text-rose-400 flex items-center gap-1">
                <AlertTriangle className="w-4 h-4" /> Cannot save with validation errors
              </span>
            )}
          </div>
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="px-4 py-2 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-semibold"
            >
              Cancel
            </button>
            <button
              ...
              onClick={handleSave}
              className="px-5 py-2 rounded bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 disabled:cursor-not-allowed text-slate-950 font-bold text-sm tracking-wide shadow"
            >
              Save Configuration
            </button>
          </div>
